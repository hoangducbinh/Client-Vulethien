...
const SigninPage = () => {
  ...
  const { isAuthenticated, setToken, setCustomer } = useAuthStore(state => ({
  ...
  useEffect(() => {
    if (isAuthenticated) {
      router.push('/customer'); // Redirect to home if already authenticated
    }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await mutateAPI('/customer/login', { email, password }, 'post');
    ...
    <div className="flex h-screen items-center justify-center bg-gradient-to-r from-blue-50 to-blue-100">
      <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-xl shadow-2xl">
        ...
        <div className="flex justify-between mt-6">
          ...
          <Link href={'/signup'} className="text-sm hover:underline"> Đăng ký tài khoản mới </Link>
        </div>
      </div>
    </div>
  );
